...
class SlowFastMobileNetV2(nn.Module):
    def __init__(self, num_classes):
        ...
        self.fast = FastMobileNetV2(num_classes=num_classes)
        self.fast = self.fast.to(torch.device('cuda:0'))
        self.slow = SlowMobileNetV2(num_classes=num_classes)
        self.slow = self.slow.to(torch.device('cuda:0'))

        # Thermal images have one channel. Replacing the first 3d convolution to match the number of channels
        self.fast.features[0] = nn.Conv3d(1, 32, kernel_size=(3, 3, 3), stride=(1, 2, 2), padding=(1, 1, 1), bias=False)
        self.slow.features[0] = nn.Conv3d(1, 32, kernel_size=(3, 3, 3), stride=(1, 2, 2), padding=(1, 1, 1), bias=False)



        self.classifier = nn.Sequential(
            nn.Dropout(0.2),
            nn.Linear(self.slow.last_channel*2, num_classes),
        )
    def forward(self,x):
        fast, laterals = self.fast(x[:,:,0:11,...])
        slow = self.slow(x[:,:,11:,...], laterals)
...
class FastMobileNetV2(nn.Module):
    def __init__(self, num_classes=1000, sample_size=224, width_mult=1.):
        ...
        self.features.append(conv_bn(3, input_channel, (1, 2, 2)))
        ...
        for t, c, n, s in interverted_residual_setting:
            ...
            lateral = nn.Conv3d(output_channel, output_channel * 2, kernel_size=(5, 1, 1), stride=(8, 1, 1), bias=False,
                                       padding=(2, 0, 0))
    ...
    def forward(self, x):
        ...
        for feature in self.features:
            x = feature(x)
            if feature is InvertedResidual:
                lat_out = self.laterals[idx](x)
...
class SlowMobileNetV2(nn.Module):
    ...
    def forward(self, x, laterals):
        ...
        for feature in self.features:
            x = feature(x)
            if feature is InvertedResidual:
                x = torch.cat([x, laterals[idx]],dim=1)
                idx += 1
